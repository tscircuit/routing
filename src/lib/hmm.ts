...
export const findRoute = ({
  ...
  for (let i = 0; i < pointsToConnect.length - 1; i++) {
    ...
    // Check if two points are more than 2x distant that the nearest point
    if (i > 0) {
      const prevPoint = pointsToConnect[i - 1]
      const prevDist = Math.sqrt(
        (pointA.x - prevPoint.x) ** 2 + (pointA.y - prevPoint.y) ** 2
      )

      if (dist > 2 * prevDist) continue
    }
  ...
  return pathFound
    ? { points, length: totalLength, width: grid.segmentSize, pathFound }
    : { pathFound: false }
}
